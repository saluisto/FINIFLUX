...
%first order
lambda = 0.04; %[1/h]
a=2; %cutoff time where RT are not equal to ET [h]
global tmm;
global alpha;
global xmin;
...
if distr == 1    %Exponential Model
    ...
    Vh(i)=depth_hz(i)*width(i)*por(i)*dx(i); %[m³]
     ...
     pdf=@(t) ((tmean(i))^-1)*exp(-t./(tmean(i)));
     Ar(i)=integral(pdf,a,inf);
     ET=@(t) (1/Ar(i)).*pdf(t+a);
          
     ETxT=@(t) t.*ET(t);    
     
     tmean_ET(i)=integral(ETxT,0,inf);
     ...
     REX = @(t) Cin(i).*NO3_decay(t).*ET(t);
               
     conv=integral(REX,0,Inf);   
     Cout(i)=conv;
     Cout_test(i)=Cin(i)*1/(tmean(i)*lambda+1);
%     
    ...
    dec_mass(i)=(Cin(i)-Cout(i))*Qh(i)*Ar(i); %[g/h]
    ...
    river_mass(i)=Q(i)*Cin(i); %[g/h]
    
    %Hyporheic Efficiency  
    DA(i)=(Cin(i)-Cout(i))/Cin(i);
...
elseif distr == 2 %Power Law 
    ...
    for i=1:observation_points-1;
        ...
        Ar(i)=integral(PPWR,low,inf);
        ET=@(t) (1/Ar(i)).*PPWR(t+low);
...
for i=1:observation_points-1; 
DA_reach(i)=tmean(i)/(1/lambda);
C_bar(i)=Cout(i)/Cin(i);
if distr==1
    DA_reach_monod(i)=tmean(i)/(K/my_max);
    C_bar_monod(i)=Cout(i)/K;
else
    DA_reach_monod=0;
    C_bar_monod(i)=0;
end
...
figure(5)
yyaxis left
scatter(DA_reach,C_bar,'MarkerEdgeColor',[0 .5 .5],'MarkerFaceColor',[0 .7 .7],'LineWidth',1.5);
xlabel('DA [-]');
ylabel('C/Cr [-]');
yyaxis right
scatter(DA_reach,dec_mass./1000,'MarkerEdgeColor',[1 .5 .5],'MarkerFaceColor',[1 .7 .7],'LineWidth',1.5);
ylabel('Mass Turnover [kg/h]');
